...
class KPN(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
                 channel_att=False, spatial_att=False, upMode='bilinear', core_bias=False, opt=None):
        ...
        # 各个卷积层定义
        # 2~5层都是均值池化+3层卷积
        self.conv1 = nn.Sequential(Basic(in_channel, 64, channel_att=False, spatial_att=False),
    ...
    # 前向传播函数
    def forward(self, data_with_est, layers=[], encode_only=False):
        """
        ...
        if len(layers) > 0:
            ...
            pred1 = self.kernel_pred(data_with_est, core, 1.0, rate=1)
            pred2 = self.kernel_pred(data_with_est, core, 1.0, rate=2)
            pred3 = self.kernel_pred(data_with_est, core, 1.0, rate=3)
            pred4 = self.kernel_pred(data_with_est, core, 1.0, rate=4)
...
#     print(KPN())
    ...
    # print(ResnetGenerator(3,3, [0,4,8,12,16]))
    # for name in ResnetGenerator.state_dict():
    #     print(name)
